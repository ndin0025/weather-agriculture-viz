...
const chartList = [
  ["#rainfall_map", "charts/rainfall_map.vg.json"],
  ["#temp_map", "charts/temp_map.vg.json"],
  ["#income_map", "charts/income_map.vg.json"],
  ["#rainfall_month_bar", "charts/rainfall_month_bar.vg.json"],
  ["#monthly_heatmap", "charts/monthly_heatmap.vg.json"],
  ["#wind_rose", "charts/wind_rose.vg.json"],
  ["#income_line", "charts/income_line.vg.json"],
  ["#cost_breakdown", "charts/cost_breakdown.vg.json"],
  ["#scatter", "charts/scatter.vg.json"],
  ["#temp_cost", "charts/temp_cost_scatter.vg.json"]
];

function renderCharts(selectedYear) {
  chartList.forEach(([elementId, chartPath]) => {
    d3.json(chartPath).then(spec => {
      if (selectedYear !== "all") {
      ...
      vegaEmbed(elementId, spec, embedOptions).catch(console.error);
    });
  });
}
